...
//CHATGPT
  ...
  Future<ChatRoomModel?> getChatroomModel(UserModel targetUser) async {
    ...
    print("UUUUUUUUUIIIIIIIIIIIIIDDDDDDDDDDDDDDd   =  $curUid");

    QuerySnapshot snapshot = await FirebaseFirestore.instance.collection("chatrooms").where("participants.$curUid", isEqualTo: true).where("participants.${targetUser.uid}", isEqualTo: true).get();
  ...
  //   final QuerySnapshot<Object?> querySnapshot = await usersRef.where("uid", isEqualTo: uid).get();
  //
  //   // Check if any documents are returned
  //   // Retrieve the first document (assuming "uid" is unique)
  //   final DocumentSnapshot<Map<String, dynamic>> documentSnapshot = querySnapshot.docs[0] as DocumentSnapshot<Map<String, dynamic>>;
  //
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      Map<String, dynamic>? userMap = await getUserDetails(widget.snap['uid']) as Map<String, dynamic>;
                      UserModel sellerUser = UserModel.fromMap(userMap);
                      ChatRoomModel? chatroommodel = await getChatroomModel(sellerUser);
                      User? currentUser = FirebaseAuth.instance.currentUser;
